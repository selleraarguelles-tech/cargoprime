...
// Cualquier estado no listado se añade al final, antes de "Sin datos".
...
interface Props {
  searchParams: Promise<{ cliente?: string; canal?: string; estado?: string }>
}

export default async function SeguimientoPage({ searchParams }: Props) {
  await auth()
  const { cliente, canal, estado } = await searchParams
  ...
  const visibles = estado ? pedidos.filter(p => etiquetaDe(p.trackingEstado) === estado) : pedidos
